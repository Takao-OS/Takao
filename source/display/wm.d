...
void initWM(const ref KernelFramebuffer fb) {
...
/// Load a bold font.
bool loadWMBoldFont(string path) {
    assert(path != null);
...
/// Load a cursive font.
bool loadWMCursiveFont(string path) {
    assert(path != null);
...
/// Load a sans font.
bool loadWMSansFont(string path) {
    assert(path != null);
...
void panicScreenWM(string message) {
...
void keyboardEventWM(bool isAlt, char c) {
    if (isAlt) {
        switch (c) {
            case 'n':
...
void mouseEventWM(int xVariation, int yVariation, bool isLeftClick, bool isRightClick) {
